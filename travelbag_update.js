...
function selectVacancy(){
  ...
  const candidates=all.filter(x=>priorityRank(x.time,ps)<ps.length);
  ...
  console.log('[TDR Restaurant Update] 時間選択:',target.time,`【第${p.index+1}希望 ${p.display}】`);
  target.a.click();
  return true;
}
...
function updateSelectButton(){
  ...
  const states=[['選択OFF','#777'],['選択回避','#fb8c00']];
