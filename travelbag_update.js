...
const VERSION='1.30', INSTALLED='__tdr_restaurant_update_installed__', PANEL_ID='__tdr_restaurant_update_panel';
...
function updatePriorityRows(){
  ...
  for(const {row,label,hour,minute} of priorityRows){
    ...
    if(!hour.value){ minute.value=''; minute.disabled=true; active=false; }
...
function adultNum(){ return document.querySelector('select[name="adultNum"]'); }
...
function fireStockReload(manual=false){
  ...
  if(!adult) return console.warn('[TDR Restaurant Update] adultNum が見つかりません');
  adult.value='1';
  console.log(`[TDR Restaurant Update] ${manual?'手動':'自動'}更新:`,new Date().toLocaleTimeString());
  triggerChange(adult);
}
...
function stopCountdown(){ clearInterval(countdownTimer); countdownTimer=null; updateAutoButton(); }
function disableAutoReloadOnNext(){
  if(!autoEnabled) return;
  autoEnabled=false;
  clearTimeout(fireTimer);
  fireTimer=null;
  nextFireAt=0;
  stopCountdown();
  console.log('[TDR Restaurant Update] 「次へ」押下: 自動1名 OFF');
}
document.addEventListener('click',e=>{
  if(e.target instanceof Element&&e.target.closest('#nextBtn')) disableAutoReloadOnNext();
},true);
function startPending(){
  if(++pendingCount===1) pendingStartedAt=Date.now();
  if(!pendingTimer) pendingTimer=setInterval(updateAutoButton,100);
  updateAutoButton();
}
function endPending(){
  pendingCount=Math.max(0,pendingCount-1);
  if(!pendingCount){ clearInterval(pendingTimer); pendingTimer=null; pendingStartedAt=0; }
  updateAutoButton();
}

function getVacancies(){
  return Array.from(document.querySelectorAll('#timeSlider li.vacancy')).map(li=>{
    const a=li.querySelector('a');
    return {li,a,time:(a?.textContent||'').trim()};
...
function scheduleAutoNext(time){
  ...
  autoNextTimer=setTimeout(()=>{
    ...
    if(!btn) return console.warn('[TDR Restaurant Update] nextBtn が見つかりません');
    console.log('[TDR Restaurant Update] 自動「次へ」:',time);
    btn.click();
  },0);
}
document.addEventListener('click',e=>{
  const a=e.target instanceof Element&&e.target.closest('#timeSlider li.vacancy a');
  if(a) scheduleAutoNext((a.textContent||'').trim());
},true);
function selectVacancy(){
  if(!selectMode) return false;
  const ps=getPriorities(), all=getVacancies();
  if(!all.length) return false;
  all.sort((a,b)=>priorityRank(a.time,ps)-priorityRank(b.time,ps)||a.time.localeCompare(b.time));
  const candidates=selectMode===1?all.filter(x=>priorityRank(x.time,ps)<ps.length):all;
  if(selectMode===1&&!candidates.length){
    console.log('[TDR Restaurant Update] 時間選択: 希望条件一致なし → 未選択');
    return true;
  }
  const target=candidates[0];
  if(!target) return false;
  const p=matchingPriority(target.time);
  console.log('[TDR Restaurant Update] 時間選択:',target.time,p?`【第${p.index+1}希望 ${p.display}】`:'【強制最早時間】');
  target.a.click();
  return true;
}
function scheduleVacancySelect(){
  ...
  const run=()=>{
    ...
    if(++n<50) setTimeout(run,20);
    else console.log('[TDR Restaurant Update] 時間選択: 空席なし');
  };
  setTimeout(run,0);
}
...
window.fetch=function(input,init){
  const url=typeof input==='string'?input:input?.url||'', timeGet=isTimeGet(url);
  if(timeGet) startPending();
  let p;
  try{ p=originalFetch.apply(this,arguments); }
  catch(e){ if(timeGet) endPending(); throw e; }
  if(timeGet) p.finally(()=>{ endPending(); scheduleVacancySelect(); }).catch(()=>{});
  return p;
};
const originalOpen=XMLHttpRequest.prototype.open, originalSend=XMLHttpRequest.prototype.send;
XMLHttpRequest.prototype.open=function(method,url){ this.__tdrRestaurantUpdateUrl=String(url||''); return originalOpen.apply(this,arguments); };
XMLHttpRequest.prototype.send=function(){
  const timeGet=isTimeGet(this.__tdrRestaurantUpdateUrl);
  if(timeGet){
    startPending();
    this.addEventListener('loadend',()=>{ endPending(); scheduleVacancySelect(); },{once:true});
  }
...
function updateAutoNextButton(){
  ...
  autoNextButton.textContent=autoNextEnabled?'次へ ON':'次へ OFF';
  autoNextButton.style.background=autoNextEnabled?'#d32f2f':'#777';
}
function makePriorityControls(){
  ...
  for(let i=0;i<3;i++){
    ...
    if(saved[i]){ const [h,m]=saved[i].split(':'); hour.value=h; minute.value=m==='--'?'':m; }
...
function createPanel(){
  ...
  autoButton=makeButton('自動OFF','#777',()=>{
    ...
    else{ clearTimeout(fireTimer); fireTimer=null; nextFireAt=0; stopCountdown(); }
    updateAutoButton();
  });
  selectButton=makeButton('選択OFF','#777',()=>{
    selectMode=selectMode===0?2:selectMode===2?1:0;
    selectToken++;
    updateSelectButton();
  });
  const priorityControls=makePriorityControls();
  autoNextButton=makeButton('次へ OFF','#777',()=>{
    autoNextEnabled=!autoNextEnabled;
    clearTimeout(autoNextTimer);
    autoNextTimer=null;
    updateAutoNextButton();
  });
  const manualButton=makeButton('1名','#198754',()=>fireStockReload(true));
  panel.append(autoButton,selectButton,priorityControls,autoNextButton,manualButton);
  document.body.appendChild(panel);
  updateSelectButton();
  updateAutoNextButton();
  updateAutoButton();
  console.log(`[TDR Restaurant Update] v${VERSION} 起動`);
}
