...
(() => {
  ...
  const RED = 0xff0000,
    ...
    YELLOW = 0xffd700,
    PURPLE = 0x9b59b6;
  const AUTO_MODE_PREFIX = 'tdr_weekly_restaurant_auto_mode_',
  ...
  const RESEARCH_KEY = 'tdr_weekly_restaurant_9am_mode_v3',
    ...
    RESEARCH_BASELINE_KEY = 'tdr_weekly_restaurant_9am_baseline_v3',
    RESEARCH_857_RUN_KEY = 'tdr_weekly_restaurant_857_run_date_v3',
    RESEARCH_900_RUN_KEY = 'tdr_weekly_restaurant_900_run_date_v3';
  const MEALS = ['朝食', '昼食', '夕食'],
    PATHS = ['/restaurant/weekReservation/', '/restaurant/ajaxNextWeekList/', '/restaurant/ajaxPreWeekList/'];
  const blocks = new Map(),
    commodityMeal = new Map(),
    mealStates = new Map(),
    snapshots = new Map(),
    latestNormalSnapshots = new Map(),
    lastKnownRanges = new Map(),
    vacancyHistory = new Map(),
    unresolved = [],
    errorNotifyHistory = new Map(),
    researchBaselineWait = new Set();
  const latestAnySnapshots = new Map();

  let panelRoot = null,
    controlRow = null,
    notifyPanel = null,
    researchPanel = null,
    researchNotifyPanel = null,
    analysisSendPanel = null,
    datePanel = null,
    dateButtons = [];
  let autoHint = '',
    researchPhaseHint = '',
    ...
    analysisFlashTimer = null;
  let researchMode = sessionStorage.getItem(RESEARCH_KEY) !== '0',
    researchWindowActive = false,
    researchNotifyMode = loadResearchNotifyMode(),
    researchRound = null,
    researchPass1Queued = false,
    researchBaselineSourceLabel = '8:57';
  ...
  let midnightReselectTimer = null;
  ...
  function fmtResearchRange(start, end) {
    ...
    const sy = +s.slice(0, 4),
      ...
      em = +e.slice(4, 6),
      ed = +e.slice(6, 8),
      sdt = new Date(sy, sm - 1, sd),
      edt = new Date(ey, em - 1, ed);
    return sy === ey
      ? `${sy}年${sm}月${sd}日（${'日月火水木金土'[sdt.getDay()]}）～${em}月${ed}日（${'日月火水木金土'[edt.getDay()]}）`
      : `${fmtDateJa(s)}～${fmtDateJa(e)}`;
  }
  function shortMD(v) {
    const s = normalizeYmd(v);
    if (s) return `${+s.slice(4, 6)}/${+s.slice(6, 8)}`;
    const raw = String(v || '').trim(),
      m = raw.match(/(\d{1,2})\D+(\d{1,2})$/);
    return m ? `${+m[1]}/${+m[2]}` : raw || '取得失敗';
  }
  function analysisDate(v) {
    const d = normalizeYmd(v);
    return d ? `${d.slice(0, 4)}/${d.slice(4, 6)}/${d.slice(6, 8)}` : String(v || '不明');
  }
  function analysisShortDate(v) {
    const d = normalizeYmd(v);
    return d ? `${d.slice(4, 6)}/${d.slice(6, 8)}` : String(v || '不明');
  }
  ...
  function loadAutoMode(meal) {
    ...
    if (v) return normalizeAutoMode(v);
    const old = localStorage.getItem(AUTO_PREFIX + meal),
      legacy = localStorage.getItem(OLD_AUTO_KEY);
    const mode = (old !== null ? old : legacy) === '0' ? 'off' : 'long';
    localStorage.setItem(AUTO_MODE_PREFIX + meal, mode);
    return mode;
  }
  function autoActive(s) {
    return s.mode !== 'off' && !s.forcedStopError && !researchWindowActive;
  }
  ...
  function analysisPeriod(ts, mode = researchNotifyMode) {
    ...
    for (let i = 0; i < hours.length - 1; i++) {
      ...
      if (ts < endAt) return { active: true, startAt, endAt };
    }
  }
  function hourSnapshotKey(ts) {
    const d = new Date(ts);
    return `${d.getFullYear()}${String(d.getMonth() + 1).padStart(2, '0')}${String(d.getDate()).padStart(2, '0')}${String(d.getHours()).padStart(2, '0')}`;
  }
  function hourSnapshotAt(ts) {
    const d = new Date(ts);
    return new Date(d.getFullYear(), d.getMonth(), d.getDate(), d.getHours(), 0, 0, 0).getTime();
  }
  function cloneFullSnapshotMap(map) {
    const copy = new Map();
    map.forEach((v, meal) => copy.set(meal, { ...v, slots: { ...v.slots } }));
    return copy;
  }
  function captureHourlySnapshot(now = Date.now()) {
    const key = hourSnapshotKey(now);
    if (key === lastHourlySnapshotKey || !latestAnySnapshots.size) return false;
    const states = cloneFullSnapshotMap(latestAnySnapshots);
    normalLogState.hourlySnapshots.push({ at: hourSnapshotAt(now), states });
    lastHourlySnapshotKey = key;
    console.log(`[${NAME}] 正時全枠状態保存：${analysisDateTime(hourSnapshotAt(now))}`);
    return true;
  }
  ...
  function createAm9LogState() {
    return { startAt: 0, endAt: 0, restaurant: '', knownMeals: new Set(), comparisons: [], errors: [] };
  }
  function seedNormalMeals() {
    if (normalLogState.active) existingMeals().forEach((m) => normalLogState.knownMeals.add(m));
  }
  ...
  function prepareNormalLog(now = Date.now()) {
    rollNormalLogPeriod(now);
    return normalLogState.active && !isMaintenance();
  }
  ...
  function recordNormalChanges(changes) {
    ...
    if (forceStopIncident.startedAt && changes.some((c) => eventCategory(c) === 'market'))
      forceStopIncident.marketChanged = true;
    if (!prepareNormalLog()) return;
    const at = Date.now();
    normalLogState.restaurant = restaurantName();
    changes.forEach((c) => {
  ...
  function buildAllSlotComparisons(prev, curr) {
    return [...new Set([...Object.keys(prev), ...Object.keys(curr)])]
      ...
      .map((slot) => {
        ...
        return {
          date,
          time,
          from: has(prev, slot) ? prev[slot] : '枠なし',
          to: has(curr, slot) ? curr[slot] : '枠なし'
        };
      });
  }
  function recordAm9Comparison(meal, base, current, phase) {
    ...
    am9LogState.comparisons.push({
      meal,
      phase,
      baseSource: base.source || '不明',
      baseAt: base.savedAt || null,
      compareAt: Date.now(),
  ...
  function formatNormalEventLine(e) {
    ...
    const parts = [
      analysisTime(e.at),
      analysisShortDate(e.date),
      e.time,
      `${statusForLog(e.from)}→${statusForLog(e.to)}`
    ];
    if (e.to === '空席' && e.vacancy) {
      if (e.vacancy.count <= 1) parts.push('初回');
      else {
  ...
  function buildNormalLogBody(state, endAt) {
    const lines = [],
      meals = [...new Set([...MEALS.filter((m) => state.knownMeals.has(m)), ...state.knownMeals])];
    meals.forEach((meal) => {
      const m = state.meals[meal] || { success: 0, errors: 0, ranges: new Set() },
        events = state.events.filter((e) => e.meal === meal).sort((a, b) => a.at - b.at),
        counts = eventCounts(events);
      lines.push(
        `【${meal}】`,
        `監視期間=${m.ranges.size ? [...m.ranges].join(',') : '未取得'}`,
        `読込成功=${m.success}`,
        `読込内訳=通常:${m.success}`,
        `エラー=${m.errors}`,
        `変化=${counts.market}`
    ...
    [...state.hourlySnapshots]
      .sort((a, b) => a.at - b.at)
      .forEach((s) => lines.push(...buildFullStateLines(`【正時全枠状態 ${analysisDateTime(s.at)}】`, s.states)));
    if (state.errors.length) {
      lines.push('【エラー詳細】');
      [...state.errors]
        .sort((a, b) => a.at - b.at)
        .forEach((e) => lines.push(`${analysisTime(e.at)}|${e.meal}|${e.error}`));
    }
    return lines;
  }
  function researchBaseLabel(source) {
    return source === '直前通常'
      ? '直前通常'
      : source === '8:57'
        ? '8:57'
  ...
  function buildAm9LogBody(state) {
    ...
    state.comparisons.forEach((r) => {
      const pair =
        r.phase === 'pass1' ? `${researchBaseLabel(r.baseSource)}→1周目` : `${researchBaseLabel(r.baseSource)}→2周目`;
      lines.push(
        `【${r.meal}｜${pair}】`,
        `調査期間=${analysisDate(r.weekStart)}-${analysisDate(r.weekEnd)}`,
        `基準=${researchBaseLabel(r.baseSource)} ${r.baseAt ? analysisTime(r.baseAt) : '時刻不明'}`,
        `比較=${r.phase === 'pass1' ? '1周目' : '2周目'} ${analysisTime(r.compareAt)}`
      );
      r.rows.forEach((x) =>
        lines.push(`${analysisShortDate(x.date)}|${x.time}|${statusForLog(x.from)}→${statusForLog(x.to)}`)
      );
      lines.push('');
    });
    lines.push('【AM9エラー】');
  ...
  function buildAm9DiscordSummary(state, endAt, csvOk) {
    ...
    if (!meals.length) lines.push('記録なし');
    meals.forEach((meal) => {
      const p1 = state.comparisons
        .filter((r) => r.meal === meal && r.phase === 'pass1')
        .reduce((n, r) => n + r.rows.filter((x) => x.from !== x.to).length, 0);
      const p2 = state.comparisons
        .filter((r) => r.meal === meal && r.phase === 'pass2')
        .reduce((n, r) => n + r.rows.filter((x) => x.from !== x.to).length, 0);
  ...
  function downloadCsv(filename, rows) {
    try {
      ...
      console.log(`[${NAME}] CSV自動保存：${filename}`);
      return true;
    } catch (e) {
  ...
  function appendHourlySnapshotCsvRows(rows, snapshot, common) {
    MEALS.forEach((meal) => {
      const v = snapshot.states.get(meal);
      if (!v) return;
      Object.keys(v.slots)
        .sort((a, b) => a.localeCompare(b))
        .forEach((k) => {
          ...
          rows.push([
            '正時状態',
            analysisDateTime(snapshot.at),
            analysisDate(date),
            meal,
            time,
            '',
            '',
            statusForLog(v.slots[k]),
            '',
            common.output,
            common.logType,
            common.logStart,
            common.logEnd,
            common.restaurant,
            analysisDate(v.weekStart),
            analysisDate(v.weekEnd)
          ]);
        });
    });
  }
  function buildNormalCsvRows(state, mode, endAt) {
    ...
    [...state.events]
      ...
      .forEach((e) => {
        ...
        const from = e.type === 'added' ? '枠なし' : statusForLog(e.from),
          to = e.type === 'deleted' ? '枠なし' : statusForLog(e.to);
        if (e.to === '空席' && e.vacancy) {
          if (e.vacancy.count <= 1) note = '初回';
          else {
            note = `再出現#${e.vacancy.count}`;
            if (e.vacancy.prevVacancyGapMs !== null)
              note += ` / 前回空席から${analysisGap(e.vacancy.prevVacancyGapMs)}`;
          }
        }
        rows.push([
          type,
          analysisDateTime(e.at),
          analysisDate(e.date),
          e.meal,
          e.time,
          from,
    ...
    [...state.hourlySnapshots].sort((a, b) => a.at - b.at).forEach((s) => appendHourlySnapshotCsvRows(rows, s, common));
    const names = [...new Set([...state.knownMeals, ...Object.keys(state.meals)])];
    names.forEach((meal) => {
      const m = state.meals[meal] || { success: 0, errors: 0, ranges: new Set() },
        events = state.events.filter((e) => e.meal === meal),
        counts = eventCounts(events);
      rows.push([
        '読込集計',
        '',
  ...
  function buildAm9CsvRows(state, endAt) {
    const rows = [
      ...
      start = analysisDateTime(state.startAt || endAt),
      end = analysisDateTime(endAt),
      restaurant = state.restaurant || restaurantName();
    state.comparisons.forEach((r) => {
      const compare = `${researchBaseLabel(r.baseSource)}→${r.phase === 'pass1' ? '1周目' : '2周目'}`,
        baseAt = r.baseAt ? analysisDateTime(r.baseAt) : '';
      r.rows.forEach((x) =>
        rows.push([
          'AM9状態',
          start,
          ...
          compare,
          baseAt,
          analysisDateTime(r.compareAt),
          analysisDate(r.weekStart),
          analysisDate(r.weekEnd),
          analysisDate(x.date),
          x.time,
          statusForLog(x.from),
          statusForLog(x.to),
  ...
  function sendNormalLogSnapshot(state, mode, endAt, requireCsv = false) {
  ...
  function resetNormalLogState(now = Date.now()) {
    normalLogState = createNormalLogState(now);
    seedNormalMeals();
  }
  function flushNormalLog(mode, forceCurrent = false) {
    ...
    if (!normalLogState.active) {
      console.log(`[${NAME}] 通常在庫差分ログ：送信対象なし`);
      return false;
    }
  ...
  function flushNormalLogForForceStop() {
    const now = Date.now(),
      old = normalLogState,
      state = old.active ? old : { ...old, active: true };
    if (!sendNormalLogSnapshot(state, '🔶強制停止', now)) return false;
    resetNormalLogState(now);
    console.log(`[${NAME}] 通常在庫差分ログ送信・リセット：🔶強制停止`);
    return true;
  }
  function hasAm9LogData() {
    return am9LogState.comparisons.length > 0 || am9LogState.errors.length > 0;
  }
  function forceStopIncidentActive(now = Date.now()) {
    const elapsed = now - forceStopIncident.startedAt;
    return (
      !!forceStopIncident.startedAt && elapsed >= 0 && elapsed < FORCE_INCIDENT_MS && !forceStopIncident.marketChanged
    );
  }
  function beginForceStopIncident(now = Date.now()) {
    if (forceStopIncidentActive(now)) return false;
    forceStopIncident = { startedAt: now, marketChanged: false, am9EmergencyHandled: false };
    return true;
  }
        ...
        JSON.stringify(
          {
            ...
            savedAt: Date.now(),
            state: normalLogState,
            lastHourlySnapshotKey
          },
          (key, value) =>
            value instanceof Map
              ? { __tdrType: 'Map', entries: [...value] }
              : value instanceof Set
                ? { __tdrType: 'Set', entries: [...value] }
                : value
        )
      );
      return true;
    } catch (e) {
      console.error(`[${NAME}] 日またぎログ退避失敗 / 自動再検索中止`, e);
      return false;
    }
  }
  function restoreMidnightCarry() {
    try {
      const raw = sessionStorage.getItem(MIDNIGHT_CARRY_KEY);
      if (!raw) return;
      const data = JSON.parse(raw, (key, value) =>
        value?.__tdrType === 'Map'
          ? new Map(value.entries)
          : value?.__tdrType === 'Set'
            ? new Set(value.entries)
            : value
      );
      ...
      if (
        ...
        now < data.savedAt ||
        now - data.savedAt > 600000 ||
        !data.state?.active ||
        now >= data.state.periodEndAt
      )
        return;
      normalLogState = data.state;
      lastHourlySnapshotKey = data.lastHourlySnapshotKey;
      console.log(`[${NAME}] 日またぎログ引継ぎ完了`);
    } catch (e) {
      console.error(`[${NAME}] 日またぎログ引継ぎ失敗`, e);
    }
  }
  function scheduleMidnightReselect() {
    ...
    midnightReselectTimer = setTimeout(() => {
      ...
      const button = dateButtons.find((b) => b.dataset.value === selected);
      if (!button) return;
      // 5秒の待機中に記録した差分も、遷移の直前に退避する。
      if (!saveMidnightCarry()) return;
      console.log(`[${NAME}] 24時CSV保存指示から5秒経過 / 選択中の日付ボタンを再実行：${selected}`);
      button.click();
    }, 5000);
  }
  function normalLogTick() {
    rollNormalLogPeriod();
  ...
  function restaurantName() {
    return (
      document.querySelector('.boxRestaurant02 .header h1.heading')?.textContent.replace(/\s+/g, ' ').trim() ||
      'レストラン名取得失敗'
    );
  }
  ...
  function scanCommodityMap() {
    document.querySelectorAll('.boxRestaurant04').forEach((box) => {
      const meal = getMealFromBox(box),
        commodity = box.querySelector('.commodityCd,.commodityCD')?.value?.trim() || '';
  ...
  function refreshBlocks() {
    ...
    document.querySelectorAll('.boxRestaurant04').forEach((box) => {
      const meal = getMealFromBox(box),
        next = box.querySelector('.nextWeekLink');
      if (!meal || !next) return;
  ...
  function compactButtonStyle(el, width, fontSize = '14px') {
    baseButtonStyle(el);
    Object.assign(el.style, { width: `${width}px`, height: '29px', padding: '4px 0', fontSize, borderRadius: '6px' });
  }
  ...
  function hidePanelCounter(s) {
    if (s.statusCount) s.statusCount.style.display = 'none';
    if (s.panel) {
  ...
  function showPanelCounter(s, meal, value, color, title) {
    ...
    s.panel.style.textAlign = 'center';
    s.panel.textContent = meal;
    s.panel.title = title;
    s.statusCount.style.background = color;
    s.statusCount.style.color = '#fff';
    s.statusCount.textContent = String(value);
    s.statusCount.style.display = 'flex';
  }
  function createPanels() {
    ...
    MEALS.forEach((meal, i) => {
      ...
      if (!s.row) {
        ...
        Object.assign(row.style, { order: String(i), display: 'flex', gap: '5px', position: 'relative' });
        ...
        Object.assign(statusCount.style, {
          position: 'absolute',
          top: '0',
          right: '0',
          width: '68px',
        ...
        row.append(manual, panel, statusCount);
        panelRoot.appendChild(row);
        s.row = row;
        s.manual = manual;
        s.panel = panel;
        s.statusCount = statusCount;
      }
  ...
  function setSearchDate(d) {
    const y = d.getFullYear(),
      ...
      day = d.getDate(),
      displayDate = `${y}/${m}/${day}`,
      searchDate = `${y}${String(m).padStart(2, '0')}${String(day).padStart(2, '0')}`,
      datepicker = document.querySelector('#datepicker'),
      searchUseDate = document.querySelector('#searchUseDate');
    ...
    const button = document.querySelector(
      '#detailFormId input[type="image"][alt="検索する"],#detailFormId input[type="image"][onclick*="Restaurant.detail"]'
    );
    if (!button) {
  ...
  function addMonthsClamped(date, months) {
    const d = new Date(date.getFullYear(), date.getMonth(), 1),
      targetMonth = d.getMonth() + months,
      targetYear = d.getFullYear() + Math.floor(targetMonth / 12),
      month = ((targetMonth % 12) + 12) % 12,
      lastDay = new Date(targetYear, month + 1, 0).getDate();
    return new Date(targetYear, month, Math.min(date.getDate(), lastDay));
  }
  function searchEndWeek() {
    const end = addMonthsClamped(new Date(), 3);
  ...
  function reservationLeadDays(v) {
    ...
    if (!s) return null;
    const now = new Date(),
      target = new Date(+s.slice(0, 4), +s.slice(4, 6) - 1, +s.slice(6, 8));
    return Math.round(
  ...
  function reservationLeadLabel(v) {
    ...
    const now = new Date(),
      ...
      target = new Date(+s.slice(0, 4), +s.slice(4, 6) - 1, +s.slice(6, 8)),
      days = reservationLeadDays(s);
    if (days === 0) return '当日';
    if (days < 0) return `${Math.abs(days)}日経過`;
    let text = `${days}日前`,
      months = (target.getFullYear() - today.getFullYear()) * 12 + target.getMonth() - today.getMonth();
  ...
  function loadPreviousAuto() {
    try {
      return JSON.parse(sessionStorage.getItem(RESEARCH_PREV_AUTO_KEY) || '{}');
    } catch {
      return {};
    }
  }
  function loadResearchBaseline() {
    try {
      const d = JSON.parse(sessionStorage.getItem(RESEARCH_BASELINE_KEY) || 'null');
      if (d?.date === ymd()) return d;
    } catch {}
    return { date: ymd(), entries: {} };
  }
  function saveResearchBaselineData(data) {
    sessionStorage.setItem(RESEARCH_BASELINE_KEY, JSON.stringify(data));
  }
  function clearResearchBaseline() {
    saveResearchBaselineData({ date: ymd(), entries: {} });
  }
  ...
  function finishResearchModeAuto() {
    const suppressIncidentLogs = forceStopIncidentActive() && forceStopIncident.am9EmergencyHandled;
    if (suppressIncidentLogs) am9LogState = createAm9LogState();
    else sendAm9StateLog();
    researchWindowActive = false;
    researchRound = null;
    researchPass1Queued = false;
    researchBaselineWait.clear();
    sessionStorage.setItem(RESEARCH_KEY, researchMode ? '1' : '0');
    restoreAutoAfterResearch();
    sessionStorage.removeItem(RESEARCH_PREV_AUTO_KEY);
    sessionStorage.removeItem(RESEARCH_BASELINE_KEY);
    console.log(`[${NAME}] 9時調査完了 / ON維持・翌日8:57待機 / 通常監視復帰`);
    renderPanels();
  }
  ...
  function saveResearchBase(meal, meta, current, source) {
    const data = loadResearchBaseline(),
      key = snapshotKey(meta, meal, current);
    data.entries[meal] = {
      source,
      key,
      weekStart: current.weekStart,
  ...
  function toggleMeal(meal) {
    ...
    if (s.forcedStopError) {
      ...
      s.retryError = false;
      s.lastError = '';
      forceStopIncident = { startedAt: 0, marketChanged: false, am9EmergencyHandled: false };
  ...
  function renderPanels() {
    ...
    MEALS.forEach((meal) => {
      ...
      if (s.pendingError) {
        showPanelCounter(s, meal, '5分', '#ff0000', 'Pending 5分超過 / 自動復帰待機');
        return;
      }
      if (s.pending > 0) {
        const elapsed = Math.floor((now - s.startedAt) / 1000),
          mode = autoModePanel(s.mode);
        showPanelCounter(s, meal, elapsed, '#7b2cbf', `${meal} 読込中 ${elapsed}秒 / 自動読込：${mode.name}`);
        return;
      }
      if (s.retryError) {
        const remain = s.deadline ? Math.max(0, Math.ceil((s.deadline - now) / 1000)) : '--',
          wait = s.deadline ? `自動復帰まで${remain}秒` : '自動復帰待機なし';
        showPanelCounter(s, meal, remain, '#ff0000', `${s.lastError || '読込エラー'} / ${wait}`);
        return;
      }
    ...
    if (researchPanel) {
      researchPanel.style.background = researchMode ? (researchWindowActive ? '#d32f2f' : '#f8bbd0') : '#000';
      researchPanel.style.color = researchMode && !researchWindowActive ? '#000' : '#fff';
      researchPanel.textContent = '9️⃣';
      researchPanel.title = researchMode
        ? researchWindowActive
          ? '9時調査 ON / 調査管理中'
          : '9時調査 ON / 8:57まで通常自動検索継続'
        : '9時調査 OFF';
    }
    renderDateSelection();
  }
  function researchRoundLabel(phase) {
    return phase === 'baseline' ? '基準' : phase === 'pass1' ? '1周' : phase === 'pass2' ? '2周' : '調査';
  }
  ...
  function forceStopError(meal, lastError) {
    ...
    if (beginForceStopIncident()) {
      ...
      if (researchWindowActive) {
        const hasData = hasAm9LogData();
        forceStopIncident.am9EmergencyHandled = !hasData || sendAm9StateLog();
      }
    } else console.log(`[${NAME}] ${meal} 同一ブロック事故の後続STOP / 追加通知・ログ抑制`);
    renderPanels();
  }
  function fireSameWeek(meal, manual = false, researchPhase = '') {
    const s = getState(meal);
    if ((researchPhase && s.forcedStopError) || (!manual && !autoActive(s)) || s.pending || isMaintenance())
      return false;
    refreshBlocks();
    const block = blocks.get(meal);
    if (!block) {
    ...
    const link = block.next,
      display = link.closest('.header')?.nextElementSibling?.querySelector('.date li:first-child .display'),
      jq = window.jQuery || window.$,
      format = window.webapiFormat;
  ...
  function researchTick() {
    ...
    if (
      ((h === 9 && m === 0) ||
        (h >= 9 && researchWindowActive && sessionStorage.getItem(RESEARCH_857_RUN_KEY) === date)) &&
      sessionStorage.getItem(RESEARCH_900_RUN_KEY) !== date
    ) {
  ...
  function runResearchRound(phase, pendingBefore = new Set()) {
    ...
    if (phase === 'pass1') sendResearchDelayDiscord();
  ...
  function finishResearchRoundIfReady() {
    ...
    if (!researchMode || !researchWindowActive) {
      renderPanels();
      return;
    }
  ...
  function readCurrentSlots(meal) {
    ...
    const dates = [...block.box.querySelectorAll('.timeList .date li .display')]
        .map((e) => e.textContent.trim())
        .filter(Boolean),
      lists = [...block.box.querySelectorAll('.timeTable .slider .view > ul.cf')];
  ...
  function compareAndSave(meta, meal, current) {
    ...
    const changes = compareSlotMaps(previous, current.slots, meal),
      next = {};
    Object.keys(current.slots).forEach((slot) => {
      next[slot] =
        current.slots[slot] === '不明' && has(previous, slot) && previous[slot] !== '不明'
          ? previous[slot]
          : current.slots[slot];
    });
  ...
  function vacancyKey(c) {
    return `${restaurantName()}|${c.date}|${c.meal}|${c.time}`;
  }
  function trackVacancyTransitions(changes) {
    const now = Date.now();
    return changes.map((c) => {
      const key = vacancyKey(c),
        h = vacancyHistory.get(key) || { count: 0, lastFullAt: null, lastVacancyAt: null };
      ...
      const gapMs = h.lastFullAt === null ? null : now - h.lastFullAt,
        notify = h.count === 1 || (gapMs !== null && gapMs >= VACANCY_RENOTIFY_MS);
      h.lastVacancyAt = now;
      h.lastFullAt = null;
      vacancyHistory.set(key, h);
  ...
  XMLHttpRequest.prototype.send = function (body) {
    ...
    if (x?.method === 'POST' && watched(x.url)) {
      const str = bodyString(body),
        ...
        commodity = params.get('commodityCD') || '',
        meal = autoHint || resolveMeal(commodity),
        meta = {
          body: str,
          commodity,
          pendingTimer: null,
          errorCounted: false,
          researchPhase: researchPhaseHint || '',
          completed: false,
          timedOut: false
        };
      if (meal) {
      ...
      meta.pendingTimer = setTimeout(() => {
        ...
        if (pendingMeal) {
          ...
          if (!s.pending) {
            ...
            if (
              !s.forcedStopError &&
              !triggerWaitingResearchBaseline(pendingMeal) &&
              autoActive(s) &&
              blocks.has(pendingMeal)
            )
              schedule(pendingMeal);
          }
          markResearchDone(pendingMeal, meta.researchPhase);
        } else {
        ...
        try {
          ...
          console.warn(`[${NAME}] Pending通信abort失敗`, e);
        }
        queueRefresh();
        renderPanels();
      }, PENDING_ERROR_MS);
  ...
  function drainUnresolved() {
    for (let i = unresolved.length - 1; i >= 0; i--) {
      const x = unresolved[i],
        meal = resolveMeal(x.meta.commodity);
      if (!meal) continue;
  ...
  function sortChanges(changes) {
    return [...changes].sort(
      (a, b) =>
        a.date.localeCompare(b.date) || MEALS.indexOf(a.meal) - MEALS.indexOf(b.meal) || a.time.localeCompare(b.time)
    );
  }
  ...
  function displayStatus(s) {
    return String(s || '不明').startsWith('◆受付終了') ? '◆受付終了' : s;
  }
  function buildAddedDescription(changes) {
    return sortChanges(changes)
      .map((c) => {
        const st = displayStatus(c.to),
          icon = st === '空席' ? '🔴' : st === '満席' ? '⚫️' : st === '◆受付終了' ? '◆' : '';
  ...
  function buildReceptionDescription(changes) {
    return sortChanges(changes)
      .map((c) => `${c.time}　${displayStatus(c.to)}`)
      .join('\n');
  }
  ...
  function timeInRange(v, start, end) {
    const t = timeMinutes(v),
      a = timeMinutes(start),
      b = timeMinutes(end);
  ...
  function sendChangesDiscord(changes) {
    ...
    groupDateMeal(deleted).forEach((g) =>
      postDeletedDiscord(
        buildTitle('🔷', g.date, g.meal),
        withReservationLead(buildDeletedDescription(g.changes), g.date),
        g.date
      )
    );
    ...
    if (notifyState.mode === 'all') {
      ...
      groupDateMeal(changes.filter((c) => c.type === 'changed' && c.to === '満席')).forEach((g) =>
        postDiscord(
          buildTitle('⚫️', g.date, g.meal),
          withReservationLead(buildTimeOnlyDescription(g.changes), g.date),
          BLACK
        )
      );
      groupDateMeal(changes.filter((c) => c.type === 'changed' && String(c.to).startsWith('◆受付終了'))).forEach((g) =>
        postDiscord(
          buildTitle('◆', g.date, g.meal),
          withReservationLead(buildReceptionDescription(g.changes), g.date),
          GRAY
        )
      );
      return;
    }
    const vacancy = changes.filter(
      (c) => c.type === 'changed' && c.to === '空席' && c.vacancy?.notify && !taken.has(c)
    );
    groupDateMeal(vacancy).forEach((g) =>
      postDiscord(
        buildTitle('🔴', g.date, g.meal),
  ...
  function sendResearchResult(meal, current, changes, pass) {
    ...
    const added = changes.filter((c) => c.type === 'added' && !taken.has(c)),
      deleted = changes.filter((c) => c.type === 'deleted'),
      vacancy = changes.filter((c) => c.type === 'changed' && c.to === '空席' && !taken.has(c)),
      full = changes.filter((c) => c.type === 'changed' && c.to === '満席'),
      closed = changes.filter((c) => c.type === 'changed' && String(c.to).startsWith('◆受付終了'));
    groupDateMeal(added).forEach((g) =>
      postCriticalDiscord(
    ...
    groupDateMeal(deleted).forEach((g) =>
      postDeletedDiscord(
        buildResearchTitle('🔷', current, g.meal),
        withReservationLead(
          `${prefix}\n変化日：${fmtDateShortJa(g.date)}\n${buildDeletedDescription(g.changes)}`,
          g.date
        ),
        g.date
      )
    ...
    if (notifyState.mode === 'off') return;
    groupDateMeal(notifyState.mode === 'all' ? vacancy : vacancy.filter((c) => c.vacancy?.notify)).forEach((g) =>
      postDiscord(
        buildResearchTitle('🔴', current, g.meal),
        withReservationLead(
    ...
    groupDateMeal(full).forEach((g) =>
      postDiscord(
        buildResearchTitle('⚫️', current, g.meal),
        withReservationLead(
          `${prefix}\n変化日：${fmtDateShortJa(g.date)}\n${buildTimeOnlyDescription(g.changes)}`,
          g.date
        ),
    ...
    groupDateMeal(closed).forEach((g) =>
      postDiscord(
        buildResearchTitle('◆', current, g.meal),
        withReservationLead(
          `${prefix}\n変化日：${fmtDateShortJa(g.date)}\n${buildReceptionDescription(g.changes)}`,
          g.date
        ),
        GRAY
      )
  ...
  function sendResearchDelayDiscord() {
    const now = Date.now(),
      d = new Date(now),
      startAt = new Date(d.getFullYear(), d.getMonth(), d.getDate(), 9, 0, 0, 0).getTime(),
      delaySec = Math.floor((now - startAt) / 1000),
      date = ymd();
    if (delaySec < 60 || researchDelayNotifiedDate === date) return;
    researchDelayNotifiedDate = date;
  ...
  async function getPublicIp() {
    const controller = new AbortController(),
      timer = setTimeout(() => controller.abort(), IP_TIMEOUT);
    try {
  ...
  async function sendResearchErrorDiscord(meal, phase, error) {
    ...
    const label =
      phase === 'baseline'
        ? `${researchBaselineSourceLabel}基準`
        : phase === 'pass1'
          ? '1周目'
          : phase === 'pass2'
            ? '2周目'
