// ==UserScript==
// @name         TDR reserve: HODHMTGD0004N + useDate=20251219 + M17 (+queue sync)
...
(() => {
  ...
  // 固定値
  const TARGET   = 'HODHMTGD0004N'; // 部屋ID（commodityCD） ← ハバグラ
  const FIX_DATE = '20251219';      // useDate（YYYYMMDD）← 25/12/19
  const FIX_PF   = 'M17';           // ランク（hotelPriceFrameID）
  ...
  console.log('[tdr-fixed] room=HODHMTGD0004N, date=20251219, rank=M17 (queue sync ON)');
})();
